...
| 2 | The TL-WR3002X may not bridge its Ethernet port onto the host subnet. TP-Link documents the mode list but not per-mode NAT or port behaviour. | Set Client mode, plug a laptop into the LAN port, then check the default gateway is the house router and that a known house device answers on its local address. Address range alone proves nothing — the house may use the same one. | Try Range Extender mode. Do not settle for Hotspot/WISP — it adds NAT and loses AirPlay. |
